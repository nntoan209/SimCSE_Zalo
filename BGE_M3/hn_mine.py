...
def find_knn_neg(model, input_file, corpus_file, candidate_pool, output_file, sample_range, negative_number, use_gpu):
    corpus = []
    with open(corpus_file, encoding='utf-8') as f:
        corpus_indexed = json.load(f)
        corpus_indexed_inverse = {v: k for (k, v) in corpus_indexed.items()}
        corpus = list(corpus_indexed.values())
    ...
    p_vecs = model.encode(corpus, batch_size=8, max_length=8192)
    print(f'inferencing embedding for queries (number={len(queries)})--------------')
    q_vecs = model.encode_queries(queries, batch_size=512, max_length=128)
    ...
    for i, data in enumerate(train_data):
        ...
        for inx in inxs:
            if inx == -1: break
            if corpus_indexed_inverse[corpus[inx]] not in data['pos'] and corpus[inx] != query:
                filtered_inx.append(inx)

        if len(filtered_inx) > negative_number:
            filtered_inx = random.sample(filtered_inx, negative_number)
        data['neg'] = [corpus_indexed_inverse[corpus[inx]] for inx in filtered_inx]
